...
class AIClient:
    ...
    def __init__(self):
        ...
        self.client = None
        self.provider = os.environ.get('AI_PROVIDER') or self.config.get('provider', 'openai')
        # 加载提示词配置
        self._init_client()
    ...
    def set_provider(self, provider: str):
        ...
        Args:
            provider: 提供商名称
        """
        if provider in LLMProviderRegistry.names():
    ...
    def analyze_user_requirement(self, user_input: str, max_tokens: Optional[int] = None) -> Optional[str]:
        ...
        # 获取当前提供商的模型配置
        provider_config = self.config.get(self.provider, {})
        model = resolve_provider_config(self.provider, provider_config).model
