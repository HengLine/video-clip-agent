...
from penclip.logger import debug, info, error
...
DEFAULT_CONFIG = {
    ...
    'ai_model': {
        'provider': 'qwen',  # 默认AI模型提供商: openai, qwen, deepseek, ollama
        'temperature': 0.1,
        'max_tokens': 2000,
        'timeout': 30,
        # 各供应商子配置仅需存在 key，真实默认值由 services/llm_config.py 的 ProviderSpec 兜底
    ...
    try:
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                # 从环境变量更新AI模型配置
                _update_config_from_env(merged_config)
                return merged_config
        else:
            # 如果配置文件不存在，返回默认配置
            default_config = DEFAULT_CONFIG.copy()
            _update_config_from_env(default_config)
...
def _update_config_from_env(config: Dict[str, Any]) -> None:
    """
    从环境变量更新 Flask 配置。

    LLM 配置的 env 注入已移至 services/llm_config.py 的 resolver，此处不再处理。
