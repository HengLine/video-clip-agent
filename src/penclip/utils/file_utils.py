...
from penclip.utils.obj_utils import dict_to_obj
...
def get_file_path(package, file_name) -> Path:
    """获取配置文件路径"""
    with resources.path(f"penclip.{package}", file_name) as path:
        return Path(path)
...
def get_subdir_path(subdir: str, filename: str) -> Path:
    ...
    with resources.path("penclip", resource_path) as path:
        return Path(path)
