...
from penclip.logger import warning, debug, info, error
try:
...
class DotEnvLoader:
    ...
    def __init__(self, package_name: str = "penclip"):
    ...
    def find_dotenv_files(self) -> List[Path]:
        ...
        3. 用户配置目录：~/.config/penclip/.env
        4. 包安装目录：site-packages/penclip/.env
...
def load_dotenv(override: bool = False) -> Dict[str, str]:
    ...
    使用示例:
        from penclip.utils.dotenv_loader import load_dotenv
        load_dotenv()  # 自动查找并加载
    """
    global _dotenv_loader
    if _dotenv_loader is None:
        _dotenv_loader = DotEnvLoader("penclip")
